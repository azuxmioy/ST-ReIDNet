...
class CustomPoseGenerator(nn.Module):
    def __init__(self, pose_feature_nc, reid_feature_nc, noise_nc, pose_nc=17, output_nc=3, 
    ...
    def forward(self, posemap, reid_feature, noise):
        ...
        if self.fuse_mode=='cat':
            feature = torch.cat((reid_feature, pose_feature),dim=1)
            ...
            if noise is not None:
                feature = feature + self.W_noise(noise.view(batch_size,-1))
