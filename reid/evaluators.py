...
def extract_features(model, data_loader, print_freq=1, feat_norm=False):
    model.eval()
...
def evaluate_all(distmat, query=None, gallery=None,
    ...
    if top1:
      ...
      if not dataset:
        ...
        if (dataset == 'cuhk03' or dataset == 'DanceReID'):
          cmc_configs = {
              'cuhk03': dict(separate_camera_set=True,
                                single_gallery_shot=True,
                                first_match_break=False),
...
class CascadeEvaluator(object):
    def __init__(self, model, embed_dist_fn=None, emb_size=2):
        super(CascadeEvaluator, self).__init__()
        self.model = model

    def evaluate(self, data_loader, query, gallery, dataset=None, top1=True):
